...
BinClock is a small app for Mac OSX that displays a binary clock: hours, minutes and seconds are displayed as bits, with decimal output on the right side for ease of reading.
...
- Start the resulting binary `BinClock`. Add an ampersand if you are running it from the commandline (or from `cron`) so that it runs in the background.
